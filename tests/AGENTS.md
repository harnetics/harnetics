...
test_release_notes.py: release notes 拼装测试，锁定安装包指引块在 GitHub release 正文中的追加与替换语义。
...
test_desktop_tauri_config.py: 桌面 Tauri 配置契约测试，锁定主窗口延迟显示、macOS DMG 安装窗口布局与 `.VolumeIcon.icns` 后处理链路。
