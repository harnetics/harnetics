...
Settings.tsx: 运行时配置页——查看和修改 LLM/Embedding 的 model、base_url、api_key、LLM thinking 开关与高级推理边界，保存后即时生效；开发者模式下右侧轮询展示后端日志。
...
Comparison.tsx: 文档比对工作台——双文件上传、四步向量流 / 传统流式切换、消费 `/api/comparison/analyze-4step` 与 `/api/comparison/analyze-stream` SSE，展示步骤状态、匹配进度、符合率与历史会话列表；历史记录删除使用应用内确认弹窗。
