...
settings.py: LLM/Embedding 运行时配置、thinking 开关、四步比对推理边界读写与开发者日志 tail 路由，挂载于 /api/settings。
status.py: 健康检查与 LLM/Embedding 状态探测路由，挂载于 /api/status。
comparison.py: 文档比对会话路由，挂载于 /api/comparison，提供 analyze、analyze-stream、analyze-4step、list、get、export、delete 端点；出口统一转北京时间并修复历史全局结论占位。
