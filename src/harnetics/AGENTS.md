...
config.py: 运行时设置对象——统一 graph DB、上传、LLM、ChromaDB、端口参数与 `.env` 解析路径（显式文件 > cwd > 仓库根）；含 RuntimeSettingsManager 内存态覆盖层。
...
api/: React SPA 主工作流入口，负责图谱栈 API、仪表盘统计与前端 dist 托管；草稿与状态端点复用统一 LLM 配置语义；settings 端点提供运行时配置读写。
