...
<directory>
desktop/ - Tauri v2 桌面壳与安装包构建模块，管理 Python sidecar、动态端口、用户数据目录和 Windows/macOS release 产物
...
desktop/AGENTS.md - 桌面打包模块地图，约束 Tauri 壳与 PyInstaller sidecar 的职责边界
.github/workflows/desktop-release.yml - 发布标签/手动触发的 Windows 与 macOS 桌面安装包构建流水线
...
.github/ - GitHub 特定配置（SECURITY.md、CI workflow、desktop release workflow、Issue/PR 模板）
</config>

目录树
- desktop/：Tauri 桌面壳 + PyInstaller sidecar 构建配置
...
- 2026-04-29: `015-comparison-multiround-stream` — 修复比对审查上下文截断 Bug（12K→240K chars，匹配模型 1M context / 384K output）；新增 `analyze_streaming` 分批生成器 + SSE 端点 `POST /api/comparison/analyze-stream`；前端流式 UI：实时进度条 + 批次意见即时展示 + 取消按钮
- 2026-04-30: `016-comparison-4step-vector` — 新增四步比对审查闭环：`/api/comparison/analyze-4step`、`Comparison4StepEngine` + `comparison_4step_support.py`、前端四步/传统双模式切换、步骤状态机、全局校验写入最终 Markdown 报告
- 2026-05-03: `017-desktop-packaging-ci` — 新增 Tauri v2 桌面壳、PyInstaller 后端 sidecar、桌面用户数据目录契约与 release CI，可为 Windows/macOS 构建安装包
...
- Python 3.12+ + TypeScript 5.7 + Rust stable + FastAPI, Uvicorn, React 18, Vite 6, Tauri v2, PyInstaller, GitHub Actions (017-desktop-packaging-ci)
- SQLite + ChromaDB + per-user app data directories (017-desktop-packaging-ci)
...
- 017-desktop-packaging-ci: Added Tauri v2 desktop shell, PyInstaller sidecar packaging, desktop runtime path helpers, and release workflow for Windows/macOS installers.
